...
export async function recordPerformance(agent) {
    ...
    // Persist to DB
    if (isDbConnected()) {
        await query(`INSERT INTO regression_snapshots (wallet_address, trust_score, success_rate, activity_score, reliability, tx_volume)
       VALUES ($1, $2, $3, $4, $5, $6)`, [key, snapshot.trustScore, snapshot.successRate, snapshot.activityScore, snapshot.reliabilityScore, snapshot.txVolume]);
    }
    // In-memory storage
 ...
 */
export async function detectRegression(agent) {
    const key = agent.walletAddress.toLowerCase();
    const alerts = [];
    ...
    let history;
    if (isDbConnected()) {
        const dbResult = await query(`SELECT trust_score, success_rate, activity_score, reliability, tx_volume, created_at
       FROM regression_snapshots
       WHERE wallet_address = $1
       ORDER BY created_at ASC
       LIMIT $2`, [key, MAX_HISTORY]);
    ...
    let regressionSeverity = 'none';
    if (regressionScore < 25)
        regressionSeverity = 'critical';
    else if (regressionScore < 50)
        regressionSeverity = 'severe';
    else if (regressionScore < 70)
        regressionSeverity = 'moderate';
    else if (regressionScore < 85)
...
// ─── Helpers ────────────────────────────────────────────────────────
function computeAverages(snapshots) {
...
export async function getRegressionStats() {
    if (isDbConnected()) {
        const agents = await query('SELECT COUNT(DISTINCT wallet_address) as count FROM regression_snapshots');
        const total = await query('SELECT COUNT(*) as count FROM regression_snapshots');
        return {
