 ...
 */
function scanGhostHunter(agent, context) {
    const alerts = [];
 ...
 */
function scanWashWatcher(agent, context) {
    const alerts = [];
    ...
    const uniqueTargets = new Set(outbound.map((tx) => tx.to));
    for (const target of uniqueTargets) {
        const inflowsFromTarget = inbound.filter((tx) => tx.from === target).length;
        const outflowsToTarget = outbound.filter((tx) => tx.to === target).length;
 ...
 */
function scanMetricFaker(agent, context) {
    const alerts = [];
    ...
    if (isRound(agent.revenue))
        roundMetricCount++;
    if (isRound(agent.grossAgenticAmount))
        roundMetricCount++;
    if (isRound(agent.walletBalance))
        roundMetricCount++;
 ...
 */
function scanClusterMapper(agent, context) {
    const alerts = [];
 ...
 */
function scanPriceManipulator(agent, context) {
    const alerts = [];
    ...
    if (offerings > 10 && actualJobs < offerings * 0.1) {
        ...
        alerts.push({
            ...
            severity: 'warning',
            confidence,
            evidence: `Agent has ${offerings} offerings but only ${actualJobs} successful jobs (${((actualJobs / offerings) *
                100).toFixed(1)}% conversion).`,
 ...
 */
function scanBaselinePoisoner(agent, context) {
    const alerts = [];
    ...
    if (newSuccessRate > oldSuccessRate * 1.3 &&
        oldSuccessRate > 0 &&
        ...
        alerts.push({
            ...
            severity: 'warning',
            confidence,
            evidence: `Success rate shifted from ${oldSuccessRate.toFixed(1)}% to ${newSuccessRate.toFixed(1)}% over time.`,
 ...
 */
function scanVolumeSpikers(agent, context) {
    const alerts = [];
    if (!context.historicalData) {
        return alerts;
    }
    ...
    const volumes = recentHistory.map((h) => h.transactionCount);
 ...
 */
function scanRevenuePhantom(agent, context) {
    const alerts = [];
    ...
    if (jobCount > 0 && revenue > aGDP * 1.5) {
        ...
        alerts.push({
            ...
            severity: 'warning',
            confidence,
            evidence: `Claimed revenue ($${revenue.toFixed(2)}) exceeds gross transaction volume ($${aGDP.toFixed(2)}).`,
 ...
 */
function scanIdentityShifter(agent, context) {
    const alerts = [];
    if (!context.historicalData) {
        return alerts;
    }
    ...
    const offerings = history.map((h) => h.offeringCount || 0);
 ...
 */
function scanFrontRunner(agent, context) {
    const alerts = [];
    if (!context.graphData || context.graphData.size === 0) {
        return alerts;
    }
    ...
    const outbound = context.graphData.get(`${address}:out`) || [];
    // Sort by timestamp
    const sorted = outbound.sort((a, b) => (a.timestamp || 0) - (b.timestamp || 0));
 ...
 */
function scanSupplyChainAuditor(agent, context) {
    const alerts = [];
    if (!context.graphData || context.graphData.size === 0) {
        return alerts;
    }
    const address = agent.walletAddress;
    const outbound = context.graphData.get(`${address}:out`) || [];
    const uniqueCounterparties = new Set(outbound.map((tx) => tx.to));
    // Check if counterparties are in allAgents and their trust scores
    const counterpartyAgents = context.allAgents.filter((a) => uniqueCounterparties.has(a.walletAddress));
    // Count how many counterparties are NEW or EMERGING (low trust)
    const lowTrustPartners = counterpartyAgents.filter((a) => a.trustTier === 'NEW' || a.trustTier === 'EMERGING');
    if (counterpartyAgents.length > 5 &&
 ...
 */
function scanRegressionStalker(agent, context) {
    const alerts = [];
    if (!context.historicalData) {
        return alerts;
    }
    ...
    const successRates = history.map((h) => h.successRate || 0);
    ...
    const variance = successRates.reduce((sum, v) => sum + Math.pow(v - mean, 2), 0) / successRates.length;
    const stdDev = Math.sqrt(variance);
    if (stdDev === 0) {
 ...
 */
export function runSentinelNetwork(agent, context) {
    const allAlerts = [];
 ...
 */
export function assembleSentinelVerdict(agent, context) {
    ...
    const overallConfidence = criticalAlerts.length > 0
 ...
 */
export function deepDive(agent, alerts, context) {
    const detailedFindings = [];
    const additionalAlerts = [];
    const riskProfile = {};
    // Analyze which sentinels flagged the agent
    const flaggingSentinels = new Map();
    ...
    if (flaggingSentinels.has('GHOST_HUNTER')) {
        ...
        detailedFindings.push('Extended dormancy-to-activity timeline suggests possible account compromise.');
    ...
    if (flaggingSentinels.has('METRIC_FAKER')) {
        ...
        detailedFindings.push('Recommend: Freeze account pending metrics verification with primary data source.');
    }
    ...
    if (flaggingSentinels.has('SUPPLY_CHAIN_AUDITOR')) {
        ...
        detailedFindings.push('Agent exclusively transacts with new/untrusted agents in suspected network.');
 ...
 */
export function assessAgent(agent, context) {
