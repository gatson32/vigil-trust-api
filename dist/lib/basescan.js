...
function getProtocolsForChain(chainId) {
...
const provenanceCache = new TTLCache(1800);
// ============================================================
//  API CLIENT
// ============================================================
function getApiKey() {
...
async function etherscanFetch(params, chainId = CHAINS.BASE.id) {
 ...
 */
export async function fetchTransactions(wallet, chainId = CHAINS.BASE.id) {
 ...
 */
export async function fetchTokenTransfers(wallet, chainId = CHAINS.BASE.id) {
    const result = await etherscanFetch({
        module: 'account',
        action: 'tokentx',
        address: wallet,
 ...
 */
export async function fetchBalance(wallet, chainId = CHAINS.BASE.id) {
    const result = await etherscanFetch({
        module: 'account',
        action: 'balance',
        address: wallet,
 ...
 */
export async function getWalletProvenance(wallet, chainId = CHAINS.BASE.id) {
    ...
    const apiKey = getApiKey();
    if (!apiKey)
        return null; // graceful degradation if key not set
    try {
        ...
        const usdcTxs = tokenTxs.filter(t => t.contractAddress.toLowerCase() === usdcAddr ||
        ...
        if (allTimestamps.length >= 3) {
            ...
            for (let i = 1; i < allTimestamps.length; i++) {
                if (allTimestamps[i] - allTimestamps[i - 1] <= 5)
                    burstTxCount++;
            }
        ...
        if (counterparties.size > 0) {
            ...
            for (const tx of successTxs) {
                ...
                if (from === walletLower && to)
                    cpCounts[to] = (cpCounts[to] || 0) + 1;
                if (to === walletLower)
                    cpCounts[from] = (cpCounts[from] || 0) + 1;
            }
        ...
        if (ageDays >= 365) {
            score += 25;
            greenFlags.push(`Wallet age: ${ageDays} days (mature)`);
        }
        else if (ageDays >= 180) {
            score += 20;
            greenFlags.push(`Wallet age: ${ageDays} days`);
        }
        else if (ageDays >= 90)
            score += 15;
        else if (ageDays >= 30)
            ...
            score += 5;
            flags.push(`Young wallet: only ${ageDays} days old`);
        }
        else {
            flags.push(`Very new wallet: ${ageDays} days old — possible sybil`);
        }
        ...
        if (txCount >= 500) {
            score += 25;
            greenFlags.push(`Heavy on-chain activity: ${txCount} txs`);
        }
        else if (txCount >= 200) {
            score += 20;
            greenFlags.push(`Active on-chain: ${txCount} txs`);
        }
        else if (txCount >= 50)
            score += 15;
        else if (txCount >= 20)
            ...
            flags.push(`Minimal on-chain activity: only ${txCount} txs`);
        }
        ...
        if (cpCount >= 50) {
            score += 20;
            greenFlags.push(`Diverse counterparties: ${cpCount} unique addresses`);
        }
        else if (cpCount >= 25)
            score += 15;
        else if (cpCount >= 10)
            ...
            flags.push(`Limited counterparties: only ${cpCount} unique addresses`);
        }
        // 4. Protocol Breadth (0-15 points)
        //    0 protocols = 0, 1 = 3, 2 = 6, 3 = 10, 4+ = 15
        if (protocolsUsed.length >= 4) {
            score += 15;
            greenFlags.push(`Multi-protocol user: ${protocolsUsed.join(', ')}`);
        }
        else if (protocolsUsed.length >= 3)
            score += 10;
        else if (protocolsUsed.length >= 2)
            score += 6;
        else if (protocolsUsed.length >= 1)
            score += 3;
        ...
        if (totalUsdc >= 10000) {
            score += 15;
            greenFlags.push(`Significant USDC flow: $${Math.round(totalUsdc).toLocaleString()}`);
        }
        else if (totalUsdc >= 1000)
            score += 10;
        else if (totalUsdc >= 100)
            score += 5;
        else if (totalUsdc > 0)
            score += 2;
        // Clamp 0-100
        ...
        // Grade
        const provenanceGrade = score >= 80 ? 'A' :
            score >= 60 ? 'B' :
                score >= 40 ? 'C' :
                    score >= 20 ? 'D' : 'F';
        // Extra flags
        if (inbound > 0 && outbound === 0)
            flags.push('Receive-only wallet: never sent a transaction');
        if (usdcIn > 0 && usdcOut === 0)
            flags.push('USDC only flows in, never out');
        if (contractCalls === 0 && txCount > 10)
            flags.push('No contract interactions despite activity — possible EOA-only transfers');
        // v1.17.0: Bot and wash trading flags
        if (botScore >= 60)
            flags.push(`Bot-like behavior detected: ${burstTxCount} burst txs, median interval ${medianTxIntervalSec}s`);
        else if (botScore >= 30)
            flags.push(`Possible automated trading: ${burstTxCount} burst txs`);
        if (botScore < 15 && txCount >= 20)
            greenFlags.push('Human-like transaction patterns');
        if (washTradingScore >= 60)
            flags.push(`Wash trading signal: ${(topCounterpartyConcentration * 100).toFixed(0)}% of txs with single counterparty`);
        else if (washTradingScore >= 30)
            flags.push(`Counterparty concentration: ${(topCounterpartyConcentration * 100).toFixed(0)}% with top address`);
        if (washTradingScore < 20 && counterparties.size >= 10)
            greenFlags.push(`Diverse trading network: ${counterparties.size} counterparties`);
        const result = {
            wallet: walletLower,
            chainId,
 ...
 */
export async function quickSybilCheck(wallet) {
