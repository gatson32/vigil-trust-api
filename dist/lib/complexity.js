...
const COMPLEXITY_TIERS = {
    trivial: { label: 'Trivial', range: [0, 1], weight: 0.5 },
    simple: { label: 'Simple', range: [1, 2], weight: 1.0 },
    moderate: { label: 'Moderate', range: [2, 4], weight: 2.5 },
    complex: { label: 'Complex', range: [4, 8], weight: 5.0 },
    expert: { label: 'Expert', range: [8, Infinity], weight: 10.0 },
};
 ...
 */
export function estimateJobComplexity(agent) {
    ...
    if (buyerRatio > 0.5)
        baseComplexity *= 1.3;
    else if (buyerRatio < 0.05)
        baseComplexity *= 0.7; // Low diversity = simple repeat tasks
 ...
 */
export function scoreComplexity(agent) {
    const { avgComplexity, distribution } = estimateJobComplexity(agent);
    // Build bucket stats
    const buckets = Object.entries(COMPLEXITY_TIERS).map(([tier, config]) => ({
        tier: tier,
        label: config.label,
        range: config.range,
    ...
    for (const [tier, count] of distribution) {
        const weight = COMPLEXITY_TIERS[tier]?.weight || 1;
    ...
    for (const [tier, count] of distribution) {
        if (count > 0) {
            const range = COMPLEXITY_TIERS[tier]?.range;
            if (range)
                maxComplexity = Math.max(maxComplexity, range[1] === Infinity ? 10 : range[1]);
        }
    }
    // === Task Complexity Score (0-100) ===
    // How sophisticated are the tasks this agent handles?
    const complexityNorm = Math.min(100, Math.round(Math.log(1 + avgComplexity) / Math.log(1 + 10) * 100));
    ...
    const hardTaskRatio = agent.successfulJobCount > 0
        ...
        : 0;
    const executionQuality = Math.min(100, Math.round(agent.successRate * (0.5 + 0.5 * hardTaskRatio) // Bonus for maintaining success on hard tasks
    ));
    // === Versatility Score (0-100) ===
    // Does the agent handle a variety of task types?
    const activeTiers = buckets.filter(b => b.count > 0).length;
    const versatility = Math.min(100, Math.round((activeTiers / 5) * 60 + // Tier diversity (up to 60 points)
        Math.min(40, agent.offeringCount * 10) // Offering variety (up to 40 points)
    ));
    // === Composite (weighted blend) ===
    const composite = Math.round(complexityNorm * 0.4 +
