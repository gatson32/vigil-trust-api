...
// ─── Behavior Store ─────────────────────────────────────────────────
const baselineProfiles = new Map(); // 90-day baselines
const recentProfiles = new Map(); // 7-day windows
...
function zScore(value, mean, stddev) {
    if (stddev === 0)
        return value === mean ? 0 : 3; // Cap at 3 if no variance
 ...
 */
export function buildProfileFromAgent(agent) {
    const now = Date.now();
    ...
    if (agent.successfulJobCount > 0) {
        ...
        if (avgTxValue < 1)
            valueDist[0] = agent.transactionCount;
        else if (avgTxValue < 100)
            valueDist[1] = agent.transactionCount;
        else if (avgTxValue < 10000)
            valueDist[2] = agent.transactionCount;
        else if (avgTxValue < 100000)
            valueDist[3] = agent.transactionCount;
        else
            valueDist[4] = agent.transactionCount;
    }
    return {
 ...
 */
export function detectAnomalies(agent) {
    ...
    if (baseline.avgInterTxTimeMs > 0 && baseline.avgInterTxTimeMs < Infinity) {
        const timingZ = zScore(recent.avgInterTxTimeMs, baseline.avgInterTxTimeMs, baseline.stddevInterTxTimeMs);
        if (timingZ > 2) {
            signals.push({
                type: 'TIMING_DRIFT',
    ...
    const hourDrift = jsDivergence(recent.activeHoursDistribution, baseline.activeHoursDistribution);
    if (hourDrift > 0.15) {
        signals.push({
    ...
    const concentrationChange = Math.abs(recent.topCounterpartyConcentration - baseline.topCounterpartyConcentration);
    ...
    const valueDrift = jsDivergence(recent.txValueDistribution, baseline.txValueDistribution);
    if (valueDrift > 0.2) {
        signals.push({
...
// ─── Utility ────────────────────────────────────────────────────────
function blendProfiles(a, b, alpha) {
    const blend = (x, y) => x * (1 - alpha) + y * alpha;
    const blendArr = (x, y) => x.map((v, i) => blend(v, y[i] || 0));
    return {
        ...a,
