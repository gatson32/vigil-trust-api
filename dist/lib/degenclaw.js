...
const leaderboardCache = new TTLCache(60);
...
export async function fetchLeaderboard() {
    ...
    if (cached)
        return cached;
    const res = await fetchJson(`${LEADERBOARD_URL}?limit=1000`);
...
export async function findAgent(query) {
    ...
    if (q.startsWith('0x') && q.length === 42) {
        const byWallet = agents.find((a) => (a.agentAddress || '').toLowerCase() === q);
        if (byWallet)
            return byWallet;
    }
    // Exact id match
...
export async function fetchAccount(wallet) {
    try {
        const raw = await fetchJson(`${TRADER_BASE}/users/${wallet}/account`);
        return {
...
export async function fetchPositions(wallet) {
    try {
        const raw = await fetchJson(`${TRADER_BASE}/users/${wallet}/positions`);
 ...
 */
export function scoreDegenClawAgent(agent) {
    ...
    if (totalPnl > 1000)
        greenFlags.push(`Net profitable: +$${totalPnl.toFixed(0)} realized`);
    else if (totalPnl < -500)
        flags.push(`Net loss: -$${Math.abs(totalPnl).toFixed(0)} realized`);
    // --- 2. Consistency (Sortino-driven, 0-100) ---
    // Sortino > 2 = excellent, 1-2 = good, 0-1 = mediocre, <0 = bad
    let consistency = 50;
    if (sortino >= 2) {
        consistency = 92;
        greenFlags.push(`Strong Sortino: ${sortino.toFixed(2)}`);
    }
    else if (sortino >= 1)
        consistency = 78;
    else if (sortino >= 0.3)
        ...
        consistency = 25;
        flags.push(`Negative Sortino: ${sortino.toFixed(2)}`);
    }
    else {
        consistency = 10;
        flags.push(`Severe downside: Sortino ${sortino.toFixed(2)}`);
    }
    ...
    if (profitFactor >= 2) {
        discipline += 50;
        greenFlags.push(`Profit factor ${profitFactor.toFixed(2)}`);
    }
    ...
    if (winRate < 0.3 && totalTrades >= 10)
        flags.push(`Low win rate: ${(winRate * 100).toFixed(0)}%`);
    if (profitFactor > 0 && profitFactor < 1 && totalTrades >= 10)
        flags.push(`Profit factor below 1.0 (losing more than winning)`);
    ...
    let capitalRisk = 65;
    if (sortino < 0 && totalPnl < 0) {
        capitalRisk -= 30;
        flags.push('Consistent losses with negative risk-adjusted returns');
    }
    if (avgRoe < -0.5) {
        capitalRisk -= 25;
        flags.push(`Severe ROE decay: ${(avgRoe * 100).toFixed(0)}%`);
    }
    if (losses > wins * 3 && totalTrades >= 15) {
        capitalRisk -= 15;
        flags.push(`Loss ratio: ${losses} losses vs ${wins} wins`);
    }
    if (sortino > 1.5 && avgRoe > 0.1) {
        capitalRisk += 20;
        greenFlags.push('Positive risk-adjusted returns');
    }
    if (profitFactor > 1.5)
        capitalRisk += 10;
    ...
    if (totalTrades >= 200)
        ...
        sampleSize = 5;
    if (totalTrades < 10)
        flags.push(`Unproven: only ${totalTrades} closed trades`);
    if (totalTrades >= 100)
        greenFlags.push(`Robust sample: ${totalTrades} closed trades`);
    ...
    const rawScore = profitability * 0.22 +
        consistency * 0.25 +
        discipline * 0.18 +
        capitalRisk * 0.25 +
        sampleSize * 0.10;
    ...
    else if (trustScore >= 58) {
        trustGrade = 'C';
        trustTier = 'DEVELOPING';
    }
    else if (trustScore >= 42) {
        trustGrade = 'D';
        trustTier = 'RISKY';
    }
    else {
    ...
    // Headline reasoning
    reasoning.push(`${agent.name} sits at rank #${perf.rank ?? '?'} with $${totalPnl.toFixed(0)} realized PnL across ${totalTrades} closed trades.`);
    reasoning.push(`Win rate ${(winRate * 100).toFixed(0)}%, profit factor ${profitFactor.toFixed(2)}, Sortino ${sortino.toFixed(2)}, avg ROE ${(avgRoe * 100).toFixed(0)}%.`);
    if (trustTier === 'SHARP') {
 ...
 */
export async function scoreByQuery(query) {
