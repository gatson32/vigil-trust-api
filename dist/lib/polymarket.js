...
import { getWalletProvenance, isBasescanConfigured, CHAINS } from './basescan.js';
 ...
 */
export async function fetchTrades(wallet, maxTrades = 2000) {
    const all = [];
    let offset = 0;
    const limit = 100;
    while (all.length < maxTrades) {
        const batch = await fetchJson(`${DATA_BASE}/trades?user=${wallet}&limit=${limit}&offset=${offset}`);
...
export async function fetchPositions(wallet) {
    const data = await fetchJson(`${DATA_BASE}/positions?user=${wallet}&limit=200`);
 ...
 */
export async function fetchAllPositions(wallet) {
    const all = [];
    for (const redeemed of ['true', 'false']) {
        let offset = 0;
        while (offset < 500) {
            const batch = await fetchJson(`${DATA_BASE}/positions?user=${wallet}&limit=100&offset=${offset}&redeemed=${redeemed}`);
            if (!Array.isArray(batch) || batch.length === 0)
                break;
            all.push(...batch);
            if (batch.length < 100)
...
async function fetchMarketResolution(conditionId) {
    ...
    try {
        const data = await fetchJson(`${CLOB_BASE}/markets/${conditionId}`, 5000);
        if (!data || !data.tokens)
            return null;
        ...
        if (resolved) {
            ...
            if (marketResolutionCache.size >= CACHE_MAX_SIZE) {
                ...
                if (firstKey)
                    marketResolutionCache.delete(firstKey);
            }
 ...
 */
async function resolveTradesViaClob(trades) {
    ...
    const resolutions = await Promise.allSettled(conditionIds.map(id => fetchMarketResolution(id)));
    const resolvedBets = [];
    for (let i = 0; i < conditionIds.length; i++) {
        const res = resolutions[i];
        if (res.status !== 'fulfilled' || !res.value || !res.value.resolved)
            continue;
 ...
 */
function extractResolvedBetsFromPositions(allPositions) {
    const bets = [];
    for (const p of allPositions) {
        const cur = p.curPrice ?? 0.5;
        // Only count clearly resolved positions
        if (cur <= 0.95 && cur >= 0.05)
            continue;
        ...
        const side = rawPrice < 0.5 ? 'BUY' : 'SELL';
        const pnl = correct
            ? size * (1 - rawPrice)
            : size * (-rawPrice);
...
function computeCalibration(bets) {
    ...
    if (betsWithTimestamps.length >= 5) {
        ...
        const timeScore = Math.max(0, Math.min(100, (Math.min(avgDays, 60) / 60) * 60 + // 60% from avg days (capped at 60 days)
...
export async function scorePolymarketTrader(wallet) {
    ...
    if (trades.length === 0 && allPositions.length === 0)
        return null;
    ...
    if (resolvedBets.length < 20 && trades.length >= 50 && uniqueMarketCount >= 10) {
        try {
            ...
            if (clobResolved.length > 0) {
                ...
                const positionBetsMap = new Map();
                const tradesByConditionId = new Map();
                ...
                resolvedBets = Array.from(allConditionIds).map(cId => positionBetsMap.get(cId) || clobBetsMap.get(cId));
            }
        }
        catch {
    ...
    // --- COMPOSITE SCORE ---
    const rawTrustScore = Math.round(calibrationDim * WEIGHTS.calibration +
    ...
    const greenFlags = [];
    if (resolvedBets.length >= 10) {
        if (calibrationReport.brierScore < 0.2)
            greenFlags.push(`Strong Brier score: ${calibrationReport.brierScore}`);
        if (calibrationReport.brierScore > 0.35)
            flags.push(`Weak Brier score: ${calibrationReport.brierScore}`);
        if (calibrationReport.overconfidenceBias > 0.1)
            flags.push(`Overconfidence bias: ${calibrationReport.overconfidenceBias.toFixed(3)}`);
        if (calibrationReport.overconfidenceBias < -0.05)
            greenFlags.push(`Conservative (underconfident) bias`);
        if (calibrationReport.skillDecomposition.skill > 60)
            greenFlags.push(`Genuine predictive skill detected (${calibrationReport.skillDecomposition.skill.toFixed(0)}/100)`);
        if (calibrationReport.skillDecomposition.luck > 70)
            flags.push(`High luck component: ${calibrationReport.skillDecomposition.luck.toFixed(0)}/100 — returns may not persist`);
        // v1.16.0: New academic-grade signals
        if (calibrationReport.brierSkillScore > 0.15)
            greenFlags.push(`Brier Skill Score: ${(calibrationReport.brierSkillScore * 100).toFixed(1)}% better than naive baseline`);
        if (calibrationReport.brierSkillScore < -0.1)
            flags.push(`Brier Skill Score: ${(calibrationReport.brierSkillScore * 100).toFixed(1)}% — performing worse than naive baseline`);
        if (calibrationReport.brierDecomposition.resolution > 0.05)
            greenFlags.push(`Strong resolution/discrimination: forecasts meaningfully diverge from base rate`);
        if (calibrationReport.brierDecomposition.resolution < 0.01 && resolvedBets.length >= 20)
            flags.push(`Low resolution: forecasts cluster near base rate — no genuine opinions`);
        if (calibrationReport.logLossSkill > 0.1)
            greenFlags.push(`Log loss skill: ${(calibrationReport.logLossSkill * 100).toFixed(1)}% better than naive (rare-event sensitive)`);
        if (calibrationReport.logLossSkill < -0.2)
            flags.push(`Log loss penalty: severe overconfidence on wrong bets detected`);
        if (calibrationReport.timeliness.timelinessScore >= 60)
            greenFlags.push(`Early mover: avg ${calibrationReport.timeliness.avgDaysBeforeResolution.toFixed(0)} days before resolution`);
    }
    // Penalty flags
    if (pennyPenalty > 0)
        flags.push(`Penny-lottery strategy: ${(pennyRatio * 100).toFixed(0)}% of bets at sub-$0.10 — score penalized by ${pennyPenalty} pts`);
    if (receiveOnlyPenalty > 0)
        flags.push(`Receive-only wallet: zero outbound transactions — possible proxy/settlement address`);
    if (pnlDivPenalty > 0)
        flags.push(`PnL integrity warning: massive divergence between API-reported and on-chain USDC flows — score penalized by ${pnlDivPenalty} pts`);
    // Live edge signals
    if (positions.length >= 3 && liveEdgeDim >= 70)
        greenFlags.push(`Strong live edge: ${positions.length} open positions trending profitable`);
    if (positions.length >= 3 && liveEdgeDim < 30)
        flags.push(`Weak live edge: open positions mostly underwater`);
    if (positions.length === 0 && resolvedBets.length === 0)
        flags.push(`No position data: nothing to score`);
    if (totalPnl > 0)
        greenFlags.push(`Net profitable: +$${Math.round(totalPnl)} total PnL`);
    if (totalPnl < -100)
        flags.push(`Net loss: -$${Math.round(Math.abs(totalPnl))} total PnL`);
    if (winRate > 0.6 && resolvedBets.length >= 10)
        greenFlags.push(`Strong win rate: ${(winRate * 100).toFixed(0)}%`);
    if (winRate < 0.4 && resolvedBets.length >= 10)
        flags.push(`Low win rate: ${(winRate * 100).toFixed(0)}%`);
    if (concentration > 0.5)
        flags.push(`Concentrated portfolio: ${(concentration * 100).toFixed(0)}% in single market`);
    if (uniqueMarkets >= 20)
        greenFlags.push(`Well-diversified: ${uniqueMarkets} unique markets`);
    ...
    if (provenance) {
        ...
        if (provenance.walletAgeDays < 7)
            flags.push(`On-chain: wallet only ${provenance.walletAgeDays} days old on ${chainLabel}`);
        if (provenance.totalTransactions < 5)
            flags.push(`On-chain: only ${provenance.totalTransactions} transactions on ${chainLabel}`);
        if (provenance.walletAgeDays >= 180)
            greenFlags.push(`On-chain: ${provenance.walletAgeDays}-day wallet history on ${chainLabel}`);
        if (provenance.protocolsUsed.length >= 3)
            greenFlags.push(`On-chain: multi-protocol user (${provenance.protocolsUsed.join(', ')})`);
        ...
        if (provenance.botScore >= 60)
            flags.push(`On-chain: bot-like trading patterns detected (bot score: ${provenance.botScore})`);
        else if (provenance.botScore >= 30)
            flags.push(`On-chain: possible automated trading (bot score: ${provenance.botScore})`);
        if (provenance.washTradingScore >= 60)
            flags.push(`On-chain: wash trading signal — ${(provenance.topCounterpartyConcentration * 100).toFixed(0)}% of txs with single counterparty`);
        if (provenance.botScore < 15 && provenance.totalTransactions >= 20)
            greenFlags.push(`On-chain: human-like trading patterns`);
        if (provenance.washTradingScore < 20 && provenance.uniqueCounterparties >= 10)
            greenFlags.push(`On-chain: diverse counterparty network`);
        reasoning.push(`On-chain verification: wallet age ${provenance.walletAgeDays} days, ${provenance.totalTransactions} txs, provenance grade ${provenance.provenanceGrade}. Bot score: ${provenance.botScore}/100, wash trading score: ${provenance.washTradingScore}/100.`);
    }
    reasoning.push(`${trades.length} total trades across ${uniqueMarkets} markets.`, `${resolvedBets.length} bets on resolved markets available for calibration scoring.`, calibrationReport.resolvedBets >= 10
        ...
        : `Insufficient resolved bets for calibration analysis.`, `Skill decomposition: ${calibrationReport.skillDecomposition.skill.toFixed(0)}% skill, ${calibrationReport.skillDecomposition.luck.toFixed(0)}% luck.`);
    // v1.16.0: Academic-grade metrics in reasoning
    if (calibrationReport.resolvedBets >= 10) {
        reasoning.push(`Brier Skill Score: ${(calibrationReport.brierSkillScore * 100).toFixed(1)}% vs naive baseline (>0% = better than always predicting base rate).`, `Brier decomposition: REL=${calibrationReport.brierDecomposition.calibration.toFixed(4)} RES=${calibrationReport.brierDecomposition.resolution.toFixed(4)} UNC=${calibrationReport.brierDecomposition.uncertainty.toFixed(4)}.`, `Log loss: ${calibrationReport.logLoss.toFixed(4)} (skill: ${(calibrationReport.logLossSkill * 100).toFixed(1)}% vs naive). Lower log loss = better calibration on rare events.`);
        if (calibrationReport.timeliness.timelinessScore > 0) {
            reasoning.push(`Timeliness: avg entry ${calibrationReport.timeliness.avgDaysBeforeResolution.toFixed(1)} days before resolution, ${(calibrationReport.timeliness.earlyMoverPct * 100).toFixed(0)}% early mover.`);
        }
    }
    // --- CONFIDENCE INTERVALS ---
    let confidenceLevel;
    let confidenceMargin;
 ...
 */
export async function crawlResolvedMarkets(maxMarkets = 100, minVolume = 50000) {
    ...
    while (markets.length < maxMarkets) {
        try {
            const batch = await fetchJson(`${GAMMA_BASE}/markets?closed=true&limit=${limit}&offset=${offset}&order=volumeNum&ascending=false`, 15000);
            if (!Array.isArray(batch) || batch.length === 0)
                break;
            // Filter by minimum volume
            const filtered = batch.filter((m) => (m.volumeNum || 0) >= minVolume);
    ...
    const wallets = new Map(discoveredWallets);
    ...
    for (const market of resolvedMarkets) {
        try {
            ...
            const trades = await fetchJson(`${DATA_BASE}/trades?conditionId=${conditionId}&limit=500`, 10000);
 ...
 */
export async function buildSkillLeaderboard(maxToScore = 200, minMarketsTraded = 3) {
    ...
    for (const e of entries)
        gradeCounts[e.trustGrade] = (gradeCounts[e.trustGrade] || 0) + 1;
 ...
 */
export async function runDiscoveryCrawl(options = {}) {
    if (crawlInProgress) {
        return { status: 'already_running', discovered: discoveredWallets.size, scored: skillLeaderboard.length, topGrade: null };
    }
    crawlInProgress = true;
    try {
        const { maxMarkets = 100, minVolume = 50000, maxToScore = 200, minMarketsTraded = 3, } = options;
