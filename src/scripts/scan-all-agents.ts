...
import { scoreAgent, type ScoredAgent } from '../lib/scoring.js';
...
interface AggregateReport {
  ...
  sybilAnalysis: {
    totalFlagged: number;
    collusionDetected: number;
    quarantineRecommended: number;
    avgSybilRiskScore: number;
    riskDistribution: {
      clean: number;    // 0-24
      low: number;      // 25-49
      medium: number;   // 50-74
  ...
  ecosystemHealth: {
    ...
    totalJobs: number;
    totalAgdp: number;
    activeAgents: number;     // active in last 7d
    dormantAgents: number;    // inactive > 30d
  };
  topAgents: ScanResult[];           // Top 20 by trust score
  highRiskAgents: ScanResult[];      // All agents with sybil risk ≥ 50
  agents: ScanResult[];              // All scanned agents
}
...
╚══════════════════════════════════════════════════╝
  ...
  while (hasMore && page <= MAX_PAGES) {
    try {
      ...
      console.error(`[SCAN] Error on page ${page}:`, (err as Error).message);
...
function generateReport(results: ScanResult[], durationMs: number, dbConnected: boolean): AggregateReport {
  ...
  for (const r of results) {
    // Trust distribution
    const tier = r.agent.trustTier as keyof typeof trustDistribution;
    if (tier in trustDistribution) trustDistribution[tier]++;
    ...
    // Activity
    if (r.metrics.accountAgeDays <= 7 || r.metrics.transactionCount > 0) activeCount++;
    if (r.metrics.accountAgeDays > 30 && r.metrics.transactionCount === 0) dormantCount++;

    // Sybil
    totalSybilRisk += r.sybil.sybilRiskScore;
    if (r.sybil.sybilRiskScore >= 25) sybilFlagged++;
    if (r.sybil.collusionDetected) collusionCount++;
    if (r.sybil.quarantineRecommended) quarantineCount++;

    if (r.sybil.sybilRiskScore < 25) riskDist.clean++;
    else if (r.sybil.sybilRiskScore < 50) riskDist.low++;
    else if (r.sybil.sybilRiskScore < 75) riskDist.medium++;
    else riskDist.high++;
...
function printSummary(report: AggregateReport) {
  console.log(`
╔══════════════════════════════════════════════════════════════╗
