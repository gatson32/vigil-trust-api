...
import { scoreAllAgents, type DegenClawRiskReport } from './degenclaw.js';
 ...
 * not dedupe).
 */
export async function writeDegenClawSnapshot(): Promise<SnapshotResult> {
  ...
  let reports: DegenClawRiskReport[];
  try {
    ...
    return {
      ...
      skipped: 0,
      db: true,
      error: `scoreAllAgents failed: ${(err as Error).message}`,
      durationMs: Date.now() - started,
      capturedAt,
    };
  }
  ...
  for (const r of reports) {
    try {
      ...
      const result = await query(
        `INSERT INTO degenclaw_snapshots (
      ...
      console.error(`[snapshot] write failed for ${r.agentName}:`, (err as Error).message);
      skipped++;
    }
  }
...
export async function getAgentHistory(
  ...
  const result = await query(
    `SELECT captured_at, trust_grade, trust_score, trust_tier, pnl, trade_count, sortino_clamped
