...
import { scoreAgent, type ScoredAgent, TIER_CONFIG } from './scoring.js';
import { assessAgent, type SentinelVerdict, type SentinelContext } from './sentinel.js';
import {
  fetchAgentByWallet,
  fetchAgentById,
} from './virtuals-client.js';
 ...
 */
export async function evaluateJob(request: EvaluationRequest): Promise<EvaluationResult> {
  ...
  try {
    ...
    if (!raw) {
      ...
      if (!rawById) {
        return buildResult(false, request, null, null,
          `Agent not found: ${request.sellerAddress}. Cannot verify seller identity.`,
          startTime);
      }
    ...
    return buildResult(true, request, null, null,
      `VIGIL scoring unavailable — approving with caution. Error: ${(err as Error).message}`,
      startTime);
  }
  ...
  if (verdict && REJECT_THREAT_LEVELS.has(verdict.threatLevel)) {
    reasons.push(
      `VIGIL Sentinel Network flagged seller as ${verdict.threatLevel} ` +
  ...
  if (scored.trustScore < REJECT_THRESHOLD) {
    reasons.push(
      `Seller trust score ${scored.trustScore}/100 (${TIER_CONFIG[scored.trustTier]?.label || scored.trustTier}) ` +
  ...
  if (scored.trustScore < WARNING_THRESHOLD) {
    reasons.push(
      `WARNING: Score is below recommended threshold of ${WARNING_THRESHOLD}. ` +
  ...
  if (verdict && WARNING_THREAT_LEVELS.has(verdict.threatLevel)) {
    reasons.push(
      `Sentinel status: ${verdict.threatLevel} — ` +
 ...
 */
export async function startEvaluatorListener(): Promise<void> {
  ...
  type AcpJobType = import('@virtuals-protocol/acp-node').AcpJob;

  const contractClient = await AcpContractClientV2.build(
    privateKey as `0x${string}`,
    parseInt(entityKeyId, 10),
  ...
  const acpClient = new AcpClientClass({
    ...
    onEvaluate: async (job: AcpJobType) => {
      ...
      const evalRequest: EvaluationRequest = {
        ...
        deliverable: (job.getDeliverable() as { type: string; value: unknown }) || { type: 'unknown', value: null },
    ...
    onNewTask: async (job: AcpJobType) => {
